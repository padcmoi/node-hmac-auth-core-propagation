...
let localClientsTimer: ReturnType<typeof setInterval> | null = null;
onMounted(() => {
  peerRedisTimer = setInterval(() => refreshPeers(), 1000);
  localClientsTimer = setInterval(() => refreshLocal(), 1000);
});
onUnmounted(() => {
  if (peerRedisTimer) clearInterval(peerRedisTimer);
  if (localClientsTimer) clearInterval(localClientsTimer);
});
